...
func TestCompositeScore(t *testing.T) {
	...
	signals := SignalScores{
		...
		EpistemicAccuracy: Signal{Raw: 0.8, Normalized: 0.8, Weight: weights.EpistemicAccuracy, HasData: true},
	...
	// Manual calculation:
	// trust:     0.75 * 0.15 = 0.1125
	// rep:       0.60 * 0.10 = 0.06
	// epistemic: 0.80 * 0.20 = 0.16
	// quality:   0.70 * 0.15 = 0.105
	// source:    0.65 * 0.10 = 0.065
	// post:      NormalizeLog(50,500) * 0.05
	// domain:    0.40 * 0.05 = 0.02
	// verify:    1.00 * 0.05 = 0.05
	// accept:    0.55 * 0.10 = 0.055
	// tenure:    0.6667 * 0.05 = 0.03333
	postNorm := NormalizeLog(50, 500)
	expected := (0.75*0.15 + 0.60*0.10 + 0.80*0.20 + 0.70*0.15 +
		0.65*0.10 + postNorm*0.05 + 0.40*0.05 + 1.0*0.05 +
		0.55*0.10 + (8.0/12.0)*0.05) * 100
...
func TestCompositeScore_MissingSignals(t *testing.T) {
	...
	partial.EpistemicAccuracy.HasData = false
...
func TestDefaultWeights_SumToOne(t *testing.T) {
	w := DefaultWeights()
	sum := w.TrustScore + w.Reputation + w.EpistemicAccuracy + w.ContentQuality +
		w.SourceReliability + w.PostCount + w.DomainExpertise + w.Verification +
		w.AcceptanceRate + w.Tenure
...
func makeUniformSignals(norm float64, w Weights, hasData bool) SignalScores {
	return SignalScores{
		...
		EpistemicAccuracy: Signal{Normalized: norm, Weight: w.EpistemicAccuracy, HasData: hasData},
